...
# ============================================================================
# Public API: schedule a job from a parsed spec (no LLM)
# ============================================================================

proc scheduleSpec*(sched: AgentScheduler, spec: ScheduleSpec, scheduleDesc: string = ""): Job =
    ## Schedule a job from an already-parsed ScheduleSpec.
    ## This bypasses parseSchedule() (LLM parsing) and is intended for simple
    ## REPL-driven scheduling like: "in 2 minutes <task>".

    let instruction = if scheduleDesc.len > 0: scheduleDesc
                      elif spec.rawInput.len > 0: spec.rawInput
                      else: spec.taskPrompt
    ...
    # 4. Register with taskman
    sched.registerWithTaskman(job)

    ic "Job scheduled (spec)", job.uid, job.taskName
    return job
