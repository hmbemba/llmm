...
# OpenAI client is currently still used for embeddings/knowledge store
...
type
  ...
  Agent* = ref object
    ## `client` remains the OpenAI client used for embeddings/knowledge store,
    ## and for the default OpenAI Responses provider.
    client*   : OpenAIClient
...
proc new*(a: Agent): Agent =
  ...
  if a.provider.isNil:
    if a.client.isNil:
      raise newException(ValueError, "Agent.client is nil and no Agent.provider was provided")
    a.provider = newOpenAIResponsesProvider(a.client)

  # NOTE: currently we still require OpenAIClient for embeddings/knowledge store.
  if a.client.isNil:
    raise newException(ValueError, "Agent.client is not initialized (required for embeddings/knowledge)")
  ...
  if a.state.knowledgeStore.isNil:
    ...
    # Create the embedding provider (OpenAI for now)
