...
# TODO
proc mcpToolJson*(): JsonNode = %*{
        "type": "mcp",
    }


# https://platform.openai.com/docs/guides/tools-shell
proc shellToolJson*(): JsonNode =
    ## Create a shell tool configuration
    %*{"type": "shell_20250305"}
proc shellTool*(): Tool = BuiltInTool shellToolJson()


# https://platform.openai.com/docs/guides/tools-apply-patch
proc applyPatchToolJson*(): JsonNode = %*{
        "type": "apply_patch",
    }
proc applyPatchTool*(): Tool = BuiltInTool applyPatchToolJson()

proc webSearchToolJson*(searchContextSize = ""): JsonNode =
...
proc webSearchTool*(searchContextSize = ""): Tool = BuiltInTool webSearchToolJson(searchContextSize)
...
proc fileSearchTool*(vectorStoreIds: seq[string]): JsonNode = %*{
        "type": "file_search",
        "vector_store_ids": vectorStoreIds,
    }

# https://platform.openai.com/docs/guides/tools-image-generation
proc imageGenerationJson*(): JsonNode = %*{"type": "image_generation"}
proc imageGenerationTool*(): Tool = BuiltInTool imageGenerationJson()

# https://platform.openai.com/docs/guides/tools-code-interpreter
proc codeInterpreterTool*(): JsonNode =
    ## Create a code interpreter tool configuration
    %*{"type": "code_interpreter_20250305"}

# https://platform.openai.com/docs/guides/tools-computer-use
proc computerUseTool*(
    displayWidth = 1024,
    displayHeight = 768,
    environment = "browser"
): JsonNode =
